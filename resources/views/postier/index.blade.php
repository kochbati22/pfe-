...
<body>
  <div class="container">
    ...
    <div class="bg-white rounded-lg shadow-md p-4">
        <h5 class="text-lg font-semibold mb-4">Liste des postiers</h5>
                ...
                <table class="w-full table-auto">
                    <thead>
                        <tr class="bg-gray-100">
                            ...
                            <th class="px-4 py-2">Solde congé</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-200">
                            <td class="px-2 py-2">{{ $postier->name }}</td>
                            <td class="px-2 py-2">{{ $postier->prenom }}</td>
                            <td class="px-2 py-2">{{ $postier->email }}</td>
                            <td class="px-2 py-2">{{ $postier->adresse }}</td>
                            <td class="px-2 py-2">{{ $postier->telephone }}</td>
                            <td class="px-2 py-2">{{ $postier->matricule }}</td>
                            <td class="px-2 py-2">
                                @if ($postier->cv)
                                <a href="{{ asset('storage/' . $postier->cv) }}" target="_blank">Voir CV</a>
                                @else
                                Aucun CV
                                @endif
                            </td>
                            <td class="px-1 py-2">{{ $postier->soldeconge }}</td>
                            <td class="px-2 py-2">
                                <a href="{{ route('postier.edit', $postier->id) }}" class="text-blue-500 hover:text-blue-700 mr-2" onclick="return confirm('Êtes-vous sûr de vouloir modifier ce postier?')">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('postier.destroy', $postier->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:text-red-700" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce postier?')">
